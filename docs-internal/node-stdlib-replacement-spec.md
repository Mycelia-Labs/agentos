...
The standalone WASM runner and Node runtime must call the same Rust host
provider before R2 exits. During extraction, a cross-runtime conformance suite proves
identical fd numbering rules, errno, readiness, stat fields, socket behavior,
process behavior, and filesystem visibility before the duplicate path is
deleted.
...
temporary match names its deleting milestone. Entries cannot be renamed or
removed without deletion proof; R7 requires an empty allowlist and zero
matches.
