...
A workflow can separate an expensive agent step from a deterministic decision. This example asks an agent to review a file, passes the findings to a quality gate, and records whether the request passed or needs work. Because the review is its own step, a replay after a crash can reuse the completed review rather than running it again.

<CodeGroup>
<CodeSnippet file="examples/workflows/quality-gate-server.ts" title="quality-gate-server.ts" />
<CodeSnippet file="examples/workflows/quality-gate-client.ts" title="quality-gate-client.ts" />
</CodeGroup>

The file must already be available in the VM. The review prompt asks the agent to start with `PASS:`, `BLOCKER:`, or `CRITICAL:`; the workflow uses that explicit marker to route the result.
